...
type RedisLister interface {
	...
	List(selector labels.Selector) (ret []*v1beta1.Redis, err error)
...
// List lists all Redises in the indexer.
func (s *redisLister) List(selector labels.Selector) (ret []*v1beta1.Redis, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1beta1.Redis))
	})
	return ret, err
}
...
type RedisNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1beta1.Redis, error)
...
// List lists all Redises in the indexer for a given namespace.
func (s redisNamespaceLister) List(selector labels.Selector) (ret []*v1beta1.Redis, err error) {
...
// Get retrieves the Redis from the indexer for a given namespace and name.
func (s redisNamespaceLister) Get(name string) (*v1beta1.Redis, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1beta1.Resource("redis"), name)
	}
	return obj.(*v1beta1.Redis), nil
}
